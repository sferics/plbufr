...
def message_structure(message: T.Any) -> T.Iterator[tuple[int, str]]:
    level = 0
    coords: dict[str, int] = collections.OrderedDict()
...
def make_message_uid(message: T.Mapping[str, T.Any]) -> tuple[T.Optional[int], ...]:
    message_uid: tuple[T.Optional[int], ...]
    ...
    descriptors: int | list[int] = message["unexpandedDescriptors"]
...
def filter_keys_cached(
    message: T.Mapping[str, T.Any],
    cache: dict[tuple[T.Hashable, ...], list[BufrKey]],
    include: T.Iterable[str] = (),
) -> list[BufrKey]:
    message_uid = make_message_uid(message)
    include_uid = tuple(sorted(include))
    filtered_message_uid: tuple[T.Hashable, ...] = message_uid + include_uid
...
def datetime_from_bufr(
    observation: dict[str, T.Any], prefix: str, datetime_keys: list[str]
) -> datetime.datetime:
...
def wmo_station_id_from_bufr(
    observation: dict[str, T.Any], prefix: str, keys: list[str]
) -> int:
...
def extract_observations(
    ...
    filters: dict[str, bufr_filters.BufrFilter] = {},
    filter_method: T.Callable = all,
    skip_na: bool = False,
    base_observation: dict[str, T.Any] = {},
    ...
    for subset in range(subset_count):
        current_observation: dict[str, T.Any]
...
def extract_message(
    ...
    uncompressed_keys: dict["str", T.Any]
...
def stream_bufr(
    ...
    keys_cache: dict[tuple[T.Hashable, ...], list[BufrKey]] = {}
